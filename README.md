...
* The modified macro `refreshDocument` maintains Microsoft Word style of the bibliography generated by Mendeley's plugin when refreshing it. Only available for Mendeley Desktop 1.x (with the MS Word plugin Mendeley Cite-O-Matic).

## Author
José Luis González García

## Overview
You need all the macros `GAUG_*` to create or remove the hyperlinks. Just add them to Microsoft Word, activate the RegExp and ADODB objects and execute any of the macros `GAUG_*`. The default custom configuration allows you to use the macros without any further modification. See the detailed instructions bellow on how to install them.
...
#### The rest of GAUG_*()
Helper functions for `GAUG_removeHyperlinksForCitations(strTypeOfExecution)`, `GAUG_createHyperlinksForCitationsAPA()` and `GAUG_createHyperlinksForCitationsIEEE()`
...
#### GAUG_*
1. Copy the code to your Microsoft Word macros to install all of the macros `GAUG_*`. **See note below if using a Mac!**
...
2. Optional! Only required if the macros ask for it. The macros `GAUG_*` make use of regular expressions; hence you need to enable the RegExp object in Microsoft Visual Basic for Applications. Open the menu “Tools” | “References” and check the box next to “Microsoft VBScript Regular Expressions 5.5”.
...
3. Optional! Only required if the macros ask for it. The macros `GAUG_*` make use of streams; hence you need to enable the ADODB object in Microsoft Visual Basic for Applications. Open the menu “Tools” | “References” and check the box next to “Microsoft ActiveX Data Objects 6.1 Library”.
